...
# Forced Command auf "borg serve --restrict-to-path /data" - voller Zugriff
# (prune/delete/compact) auf jedes Repo unter /data, aber weiterhin keine
# Shell und kein Ausbruch aus /data.
#
# Usage: ./add-admin-key.sh <name> <pubkey-datei>

set -euo pipefail

BASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
NAME="${1:?Usage: add-admin-key.sh <name> <pubkey-datei>}"
PUBKEY_FILE="${2:?Usage: add-admin-key.sh <name> <pubkey-datei>}"
